...
const updateProductAction = async (state: ProductActionState | null, formData: FormData) => {
  try {
...
export default function ProductEditForm({ storeId, categories, product }: ProductEditFormProps) {
  
  const [uploadedImages, setUploadedImages] = useState<{ url: string; id?: number }[]>(
    product.images.map(img => ({ url: img.url, id: img.id }))
  );
  const [isDeleting, setIsDeleting] = useState<number | null>(null);
  const [state, formAction, pending] = useActionState(updateProductAction, { message: '', success: false });
